...
const
    ...
    arenas = [91, 84, 77, 71, 63, 55, 47, 39, 31, 25, 19, 13],
...
if (localStorage.getItem('ddType') !== null) ddType.selectedIndex = localStorage.getItem('ddType')
...
function changeDeck() {
    ...
    for (let i = 1; i < selectedArena; i++) {
        if (ddRarity.selectedIndex === 0) allowedCards.push(i)
        else if (ddRarity.selectedIndex === 1 && cardsInformation[i].split('<br />')[0] === 'Rarity: Common') allowedCards.push(i)
        else if (ddRarity.selectedIndex === 2 && cardsInformation[i].split('<br />')[0] === 'Rarity: Rare') allowedCards.push(i)
        else if (ddRarity.selectedIndex === 3 && cardsInformation[i].split('<br />')[0] === 'Rarity: Epic') allowedCards.push(i)
        else if (ddRarity.selectedIndex === 4 && cardsInformation[i].split('<br />')[0] === 'Rarity: Legendary') allowedCards.push(i)
    }

    for (let i = 0; i < allowedCards.length; i++) {
        if (ddType.selectedIndex === 1 && cardsInformation[allowedCards[i]].split('<br />')[1] !== 'Type: Troop') allowedCards[i] = 0
        else if (ddType.selectedIndex === 2 && cardsInformation[allowedCards[i]].split('<br />')[1] !== 'Type: Construction') allowedCards[i] = 0
    ...
    for (let i = 1; i < cardsName.length; i++) {
        qtdAll++
        if (cardsInformation[i].split('<br />')[0] === 'Rarity: Common') qtdCommon++
        if (cardsInformation[i].split('<br />')[0] === 'Rarity: Rare') qtdRare++
        if (cardsInformation[i].split('<br />')[0] === 'Rarity: Epic') qtdEpic++
        if (cardsInformation[i].split('<br />')[0] === 'Rarity: Legendary') qtdLegendary++
        if (cardsInformation[i].split('<br />')[1] === 'Type: Troop') qtdTroop++
        if (cardsInformation[i].split('<br />')[1] === 'Type: Spell') qtdSpell++
        if (cardsInformation[i].split('<br />')[1] === 'Type: Construction') qtdConstruction++

        if (localStorage.getItem(cardsName[i]) === 'e') {
            all++
            if (cardsInformation[i].split('<br />')[0] === 'Rarity: Common') common++
            if (cardsInformation[i].split('<br />')[0] === 'Rarity: Rare') rare++
            if (cardsInformation[i].split('<br />')[0] === 'Rarity: Epic') epic++
            if (cardsInformation[i].split('<br />')[0] === 'Rarity: Legendary') legendary++
            if (cardsInformation[i].split('<br />')[1] === 'Type: Troop') troop++
            if (cardsInformation[i].split('<br />')[1] === 'Type: Spell') spell++
            if (cardsInformation[i].split('<br />')[1] === 'Type: Construction') construction++
        }
    }

    chkConfigText[0].textContent = `All cards (${all})`
    chkConfigText[1].textContent = `Common cards (${common})`
    chkConfigText[2].textContent = `Rare cards (${rare})`
    chkConfigText[3].textContent = `Epic cards (${epic})`
    chkConfigText[4].textContent = `Legendary cards (${legendary})`
    chkConfigText[5].textContent = `Troop cards (${troop})`
    chkConfigText[6].textContent = `Spell cards (${spell})`
    chkConfigText[7].textContent = `Construction cards (${construction})`

    if (all === qtdAll) chkConfig[0].checked = true
    else chkConfig[0].checked = false

    if (common === qtdCommon) chkConfig[1].checked = true
    else chkConfig[1].checked = false

    if (rare === qtdRare) chkConfig[2].checked = true
    else chkConfig[2].checked = false

    if (epic === qtdEpic) chkConfig[3].checked = true
    else chkConfig[3].checked = false

    if (legendary === qtdLegendary) chkConfig[4].checked = true
    else chkConfig[4].checked = false

    if (troop === qtdTroop) chkConfig[5].checked = true
    else chkConfig[5].checked = false

    if (spell === qtdSpell) chkConfig[6].checked = true
    else chkConfig[6].checked = false

    if (construction === qtdConstruction) chkConfig[7].checked = true
    ...
    while (allowedCards.indexOf(0) !== -1) {
        for (let i = 0; i < allowedCards.length; i++) {
            if (allowedCards[i] === 0) allowedCards.splice(i, 1)
        }
    }
}
...
function buildDeck() {
    ...
    if (cbDeckInteligente.checked) {
        const
            typeSelected = tipos[parseInt(Math.random() * tipos.length)],
        ...
        for (let i = 0; i < typeSelected.split('|').length; i++) {
            for (let j = 0; j < troop.length; j++) {
                if (typeSelected.split('|')[i] === troop[j]) {
                    ...
                    break
                } else if (j === troop.length - 1) currentDeck.push(typeSelected.split('|')[i])
            }
        }

        for (let i = 0; i < currentDeck.length; i++) {
            for (let j = 1; j < cardsInformation.length; j++) {
                if (currentDeck[i] === capitalize(cardsName[j])) currentDeck[i] = j
            }
        }
        ...
        for (let i = 0; i < currentDeck.length; i++) prevDeck[prevDeck.length - 1].push(currentDeck[i])

        if (prevDeck.length > 100) prevDeck.shift()
        else if (prevDeck.length > 1) btnVoltar.hidden = false
    } else {
        ...
        for (let i = 0; i < (allowedCards.length < 8 ? allowedCards.length : cards.length); i++) {
            while (currentDeck.indexOf(random) !== -1) random = allowedCards[parseInt(Math.random() * allowedCards.length)]
...
function combination() {
    ...
    for (let f = i; f > (i - b); f--) iFactorial *= f

    for (let k = b; k > 0; k--) bFactorial *= k

    const formatter = Intl.NumberFormat('en-US')

    return allowedCards.length === 0 ? 0 : formatter.format(iFactorial / bFactorial)
}
...
function copyDeck() {
    if (prevDeck.length > 0) prevDeck.pop()
...
function paste(linkDeck = String) {
    ...
    if (linkDeck !== null && linkDeck.trim().indexOf(' ') === -1 && linkDeck.indexOf(';') !== -1) {
        try {
            if (linkDeck.match(/&id=/)) linkDeck = linkDeck.substring(0, linkDeck.lastIndexOf('&'))
            ...
            for (let i = 0; i < linkDeck.length; i++) {
                for (let j = 0; j < cardsCode.length; j++) {
                    if (parseInt(linkDeck[i]) === cardsCode[j]) linkDeck[i] = j
                }
            }

            for (let i = linkDeck; i < 8; i++) linkDeck.push(0)
...
function pasteDeck(content = null) {
    if (content === null) {
        paste(prompt('Paste a Deck link below\nE.g: https://link.clashroyale.com/deck/en?deck=26000018;28000009;26000003;28000008...'))
    } else paste(content)
}

function formatText(text) {
    const
        alpha = 'aaaaeeeeiiiioooouuuu',
        accentuation = 'áâàãéêèẽíìîĩóôòõúûùũ'

    text = text.replace(/[!@#$%&*()-=_+'"[\]{},.<>;:/?^~]/g, '')

    for (let i = 0; i < alpha.length; i++) {
        while (text.indexOf(accentuation[i]) !== -1) text = text.replace(accentuation[i], alpha[i])
    }
...
function previousDeck() {
    for (let i = 0; i < currentDeck.length; i++) currentDeck[i] = prevDeck[prevDeck.length - 2][i]
...
function randomizeDeck() {
    if (currentDeck.indexOf(0) === -1) {
        ...
        for (let i = 0; i < currentDeck.length; i++) {
            while (newDeck.indexOf(num) !== -1) num = currentDeck[parseInt(Math.random() * currentDeck.length)]

            newDeck.push(num)
        }

        for (let i = 0; i < newDeck.length; i++) currentDeck[i] = newDeck[i]
...
function showSections(event) {
    ...
    if (navSection.style.width !== '100%') {
        ...
        if (selectedContainer === 0) dbSection.style.display = 'none'
        else if (selectedContainer === 1) playerSection.style.display = 'none'
        else if (selectedContainer === 2) selectSection.style.display = 'none'
        else if (selectedContainer === 3) savedSection.style.display = 'none'
        else if (selectedContainer === 4) bestSection.style.display = 'none'
        else if (selectedContainer === 5) chestSection.style.display = 'none'
        else if (selectedContainer === 6) configSection.style.display = 'none'
        ...
        if (selectedContainer === 0) dbSection.style.display = 'block'
        else if (selectedContainer === 1) playerSection.style.display = 'block'
        else if (selectedContainer === 2) selectSection.style.display = 'block'
        else if (selectedContainer === 3) savedSection.style.display = 'block'
        else if (selectedContainer === 4) bestSection.style.display = 'block'
        else if (selectedContainer === 5) chestSection.style.display = 'block'
        else if (selectedContainer === 6) configSection.style.display = 'block'
...
async function downDecks() {
    ...
    if ((maxDown + 10 > response.length ? response.length - maxDown : 10) === 0) btnMore.style.display = 'none'
...
function toTop() {
    ...
    const interval = setInterval(() => {
        if (scrollYN >= 100) scrollTo(0, scrollYN -= (100 * (scrollY / 1000)))
        else {
            scrollTo(0, 0)
...
for (let i = 1; i < cardsInformation.length; i++) {
    if (localStorage.getItem(cardsName[i]) === null) localStorage.setItem(cardsName[i], 'e')
}
...
selectSection.innerHTML += `${content}<img title="To top" onclick="toTop()" src="./images/uparrow.png" class="upArrow"/>`
...
createDecks.onmessage = e => {
    savedDecks.innerHTML =
        `${e.data.html}${e.data.amount > 5 ? '<img style="margin-top: 5px" title="To top" onclick="toTop()" src="./images/uparrow.png" class="upArrow"/>' : ''}`
...
function saveDeck(deck = currentDeck) {
    let
        empty = deck.indexOf(0) === -1 ? false : true,
...
function showChests() {
    ...
    fetch(`https://api.royaleapi.com/player/${idUser.value.trim().replace('#', '')}/chests`, settings).then(data => data.json()).then(response => {
        if (localStorage.getItem('id') === null) localStorage.setItem('id', idUser.value.trim())
        ...
        if (playerInfo !== null) {
            try {
                if (playerInfo.length === cardsName.length - 1) {
                    ...
                    const cards = []
                    for (let card in playerInfo) cards.push(playerInfo[card].key)
...
for (let i = 1; i < cardsName.length; i++) cardName += `<option value="${capitalize(cardsName[i])}"></option>`
    ...
    if (e.which === 13) {
        ...
        for (let i = 1; i < cardsName.length; i++) {
            if (value === cardsName[i].split('-').map(name => name).join(' ').toLowerCase() || value === formatText(cardPtName[i].toLowerCase())) {
                ...
                let currentY = 0
                const
                    y = img[i - 1].parentElement.offsetTop + (innerWidth < 768 ? 33 : -1),
                    interval = setInterval(() => {
                        currentY = scrollY
                        if (scrollY < y) scrollTo(0, scrollY + 25)
                        else {
                            scrollTo(0, y)
                            clearInterval(interval)
                        }

                        if (currentY === scrollY) clearInterval(interval)
                    }, 1)
    ...
    cards[i].addEventListener('contextmenu', () => {
        ...
        if (name !== null && name !== '' && name !== '0') {
            if (!Number.isInteger(parseInt(name))) {
                ...
                if (currentDeck.indexOf(name) === -1 && name > 0 && name < cardsName.length) currentDeck[i] = name
...
document.onkeydown = e => {
    if (dbSection.style.display === 'block') {
        if (e.which === 66) buildDeck()
        else if (e.which === 83) saveDeck()
        else if (e.ctrlKey && e.which === 86) pasteDeck()
    } else if (cntConfig.style.display === 'block' && e.which === 27) showConfig()
    else if (navSection.style.width === '100%' && e.which === 27) showSections(event)
...
selectSection.onclick = () => {
    if (cntConfig.style.display === 'block') showConfig()
}
...
window.onload = function () {
    ...
    if (location.search.startsWith('?deck=')) paste(location.search.substring(6, location.search.length))
}
